...
def count_medical_students():
    ...
    query = """
    MATCH (a:applicant_5000)
    WHERE a.degree = 'Medical' AND a.gender = 'Female' AND a.nationality = 'USA' AND a.institution_name = 'nyc'
...
def get_transcript_ids():
    ...
    MATCH (a:applicant_5000)
    WHERE a.gender = 'male' 
      AND a.year_of_graduation = 2010
      AND a.application_status = 'submitted'
      AND a.gpa > 8
    RETURN a.transcript_id AS transcript_id
    """
    transcript_ids = []
    with driver.session() as session:
        result = session.run(query)
...
def get_applicant_ids():
    ...
    AND a.institution_name = 'messina'
    AND a.degree = 'data analyst'
    AND a.nationality = 'italy'
    RETURN a.applicant_id AS applicant_id
    """
